...
type NavItem = { key: ActiveKey; href: string; label: string; permission?: Permission; anyPermission?: Permission[]; adminOnly?: boolean };

const items: NavItem[] = [
  { key: 'visao', href: '/', label: 'Visão geral', permission: 'ver_visao_geral' },
  { key: 'atendimento', href: '/atendimento', label: 'Atendimento', permission: 'ver_atendimento' },
  { key: 'veiculos', href: '/veiculos', label: 'Veículos', anyPermission: ['ver_todos_veiculos', 'ver_veiculos_setor'] },
  { key: 'tarefas', href: '/tarefas', label: 'Tarefas', anyPermission: ['ver_todas_tarefas', 'ver_proprias_tarefas'] },
  { key: 'funcionarios', href: '/funcionarios', label: 'Funcionários', permission: 'ver_funcionarios' },
  { key: 'acessos', href: '/acessos', label: 'Perfis e acessos', adminOnly: true },
  { key: 'configuracoes', href: '/configuracoes', label: 'Configurações', permission: 'ver_configuracoes' },
];
  ...
  return (
    <main className="app-shell">
      <aside className="sidebar">
        <Link href="/inicio" className="brand"><div className="brand-mark">PS</div><div><strong>PintService</strong><span>Pint Services · car center</span></div></Link>
        <nav className="nav" aria-label="Navegação principal">
          {visibleItems.map((item) => <Link key={item.key} className={active === item.key ? 'active' : ''} href={item.href}><i className="nav-icon" aria-hidden="true" /><span>{item.label}</span></Link>)}
          {user?.perfil === 'admin' && <Link href="/simulador"><i className="nav-icon" aria-hidden="true" /><span>Simulador</span></Link>}
        </nav>
