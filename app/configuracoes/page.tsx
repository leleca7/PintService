...
export default async function SettingsPage() {
  ...
  const connections = [
    { name: 'Neon Postgres', description: 'Guarda a fonte oficial da operação, clientes, conversas, tarefas, equipe, perfis e auditoria do Sistema da Pint.', ready: isNeonConfigured() && data.source === 'live', detail: 'DATABASE_URL — segredo do servidor' },
    ...
    { name: 'Fonte externa opcional', description: 'Google Sheets/CSV pode ser usado para importação, transição ou contingência. O banco nativo continua sendo a fonte oficial da operação.', ready: !vehicleSource.configured || vehicleSourceReady, detail: vehicleSource.error ? `fonte opcional com erro: ${vehicleSource.error}` : vehicleSourceReady ? `${vehicleSource.vehicles.length} veículo(s) lido(s) agora` : 'não configurada — e não é necessária para o core' },
    ...
    <AppShell active="configuracoes" source={data.source}>
      <div className={core.page}>
        ...
        <section className={admin.infoGrid}>
          ...
          <article className={admin.infoCard}><p>DADOS OPERACIONAIS</p><h2>Fonte de verdade</h2><ul><li>O banco nativo do Sistema da Pint é a fonte oficial da operação.</li><li>Veículos, status, tarefas e histórico devem ser atualizados uma vez e reutilizados pelo restante do sistema.</li><li>Google Sheets/CSV é opcional para importação, transição ou contingência.</li><li>Quando um dado oficial estiver ausente ou conflitante, o sistema cria uma exceção para confirmação humana em vez de inventar uma resposta.</li></ul></article>
