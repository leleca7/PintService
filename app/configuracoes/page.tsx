...
export default async function SettingsPage() {
  ...
  const connections = [
    ...
    { name: 'WhatsApp Cloud API', description: 'Recebe e envia mensagens pelo número oficial, cria tarefas e envia alertas internos.', ready: configured(process.env.WHATSAPP_ACCESS_TOKEN, process.env.WHATSAPP_PHONE_NUMBER_ID, process.env.WHATSAPP_VERIFY_TOKEN, process.env.WHATSAPP_APP_SECRET), detail: 'token + phone number id + verify token + app secret' },
  ...
  return (
    <AppShell active="configuracoes" source={data.source}>
      ...
      <section className="settings-grid">
        <article className="panel settings-card"><p className="eyebrow">CADASTRO OFICIAL</p><h2>{office.name}</h2><ul><li><strong>Telefone:</strong> {office.publicPhone}</li><li><strong>Endereço:</strong> {office.address}</li><li><strong>Horários:</strong> {office.hours}</li><li><strong>Instagram:</strong> <a href={office.instagramUrl} target="_blank" rel="noreferrer">{office.instagramHandle}</a></li><li><strong>Google:</strong> <a href={office.googleBusinessUrl} target="_blank" rel="noreferrer">abrir perfil/localização</a></li><li><strong>Site antigo:</strong> {office.legacySiteUrl} — referência histórica, atualmente indisponível.</li><li><strong>Reclame Aqui:</strong> URL recebida, mas a titularidade do perfil ainda precisa ser confirmada antes de tratá-lo como canal oficial.</li></ul></article>
